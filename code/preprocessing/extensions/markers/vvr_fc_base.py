"""Provide base class for voxel- and Region-wise denoising."""
...
class VvRConnectivityBase(BaseMarker):
    ...
    def _pick_confounds(self, input: Dict[str, Any]):
        ...
        files stored in '/data/confounds/hcp_ya_confounds/' directory.
        ...
        conf_dir = os.path.join(
            "../../../..", "data", "confounds", "hcp_ya_confounds"
        )
        assert os.path.isdir(conf_dir), "Make sure you provided the right path."

        confound_path = os.path.join(conf_dir, confound_file)
        ...
        confounds = np.nan_to_num(
            get_confounds(confounds_names, confound_path)
        )
...
def get_first_eigen_ts(parcel_data, demean=False):
    ...
    # Ensure that the input matrix is a numpy array
    ...
    # Compute the final eigenvariate time series
    eigents = np.dot(u, np.sqrt(np.divide(s[0], nvoxels)))

    return eigents


def get_confounds(conf_list, confounds_file):
